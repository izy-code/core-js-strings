...
function removeFirstOccurrences(str, value) {
  const firstOccurrenceIndex = str.indexOf(value);
  if (firstOccurrenceIndex === -1) return str;
  return (
    str.slice(0, firstOccurrenceIndex) +
    str.slice(firstOccurrenceIndex + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const lastOccurrenceIndex = str.lastIndexOf(value);
  if (lastOccurrenceIndex === -1) return str;
  return (
    str.slice(0, lastOccurrenceIndex) +
    str.slice(lastOccurrenceIndex + value.length)
  );
}
...
function countVowels(str) {
  return [...str.matchAll(/[aeiouy]/gi)].length;
}
...
function isPalindrome(str) {
  const formattedString = str
    ...
    .toLowerCase()
    .replaceAll(/\W+/g, '');
  for (let i = 0; i < formattedString.length / 2; i += 1) {
...
function unbracketTag(str) {
  return removeLastOccurrences(str.replace('<', ''), '>');
}
...
function encodeToRot13(str) {
  const getAlphabetIndex = (char) =>
    char.toLowerCase().charCodeAt(0) - 'a'.charCodeAt(0);
  const getAlphabetIndexShift = (alphabetIndex) =>
    ((alphabetIndex + 13) % 26) - alphabetIndex;
      ...
      isLetter(currentChar)
        ? String.fromCharCode(
            currentChar.charCodeAt(0) +
              getAlphabetIndexShift(getAlphabetIndex(currentChar))
          )
        : currentChar
    )
    .join('');
